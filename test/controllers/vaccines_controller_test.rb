...
class VaccinesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create vaccine" do
    assert_difference("Vaccine.count") do
      post vaccines_url, params: { vaccine: { name: @vaccine.name, status: @vaccine.status } }
    end
  ...
  test "should update vaccine" do
    patch vaccine_url(@vaccine), params: { vaccine: { name: @vaccine.name, status: @vaccine.status } }
